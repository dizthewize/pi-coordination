...
- **Coordinator tools not passed to coordinator agent** — `coordinate()` dispatched 0 workers because the Pi SDK's `customTools` don't translate to native Anthropic tool schemas. The model (claude-opus-4-5) output `<tool_call>` tags as TEXT instead of using native `tool_use` blocks, so the SDK treated all tool "calls" as plain text — no actual tools executed. Fixed by forcing **subprocess mode** for the coordinator agent, which loads the coordinator extension (`coordinator.ts`) via the Pi CLI. The CLI properly registers tools with the model's native API, so `spawn_workers`, `create_contract`, `done`, etc. are actually executed.
