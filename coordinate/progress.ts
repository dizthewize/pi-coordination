	...
	if (config.includeDetailedHistory) {
		...
		if (reviewHistory && reviewHistory.length > 0) {
			...
			for (let i = 0; i < historyToShow.length; i++) {
				...
				lines.push(`- **Duration:** ${Number.isFinite(review.duration) ? (review.duration / 1000).toFixed(1) : "—"}s`);
		...
		for (const ev of recentEvents) {
			...
			switch (ev.type) {
				...
				case "worker_failed":
					...
					break;
				case "phase_complete":
					desc = `Phase ${ev.phase} complete ($${Number.isFinite(ev.cost) ? ev.cost.toFixed(2) : "—"})`;
					break;
				case "cost_limit_reached":
					desc = `Cost limit reached: $${Number.isFinite(ev.total) ? ev.total.toFixed(2) : "—"}`;
					break;
				default:
					desc = ev.type;
			}
